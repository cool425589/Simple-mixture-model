...
def init_subfilename():
    subdocument_name[:] = Get_subfile_list('Homework4/Document')
    subquery_name[:] = Get_subfile_list('Homework4/Query')    
...
def Get_rank_result():
    init()
    init_subfilename()  
    init_count_by_FolderName('Homework4/Document', count_d_T_dic)
    print ("count Document tf")
    init_tf_by_FolderName('Homework4/Document', d_T_dic)
    print ("\ncount Query tf")
    init_tf_by_FolderName('Homework4/Query', q_T_dic)
    print ("\ncount word idf")
    init_idf()         
    write_result_path = os.path.join(os.path.sep,os.getcwd(),'VSM_result.txt')
    print (write_result_path)
    rank_result = cosine_similarity(q_T_dic, d_T_dic*idf_dic[np.newaxis,:]) 
    with open(write_result_path, 'w', encoding = 'UTF-8') as write_file :
        write_file.write('Query,RetrievedDocuments')
        print("\nfirst start ranking")
        rank_ALL_func(write_file, rank_result)(np.arange(query_size))
        write_file.close()    
    return rank_result
